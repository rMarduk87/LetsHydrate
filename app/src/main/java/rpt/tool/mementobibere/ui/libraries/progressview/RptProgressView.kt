package rpt.tool.mementobibere.ui.libraries.progressview
...
class RptProgressView @JvmOverloads constructor(
    ...
    private var rectRadius: Float by OnValidateProp(5F.pxValue())


    private var textMargin: Float by OnValidateProp(10F.pxValue())


    private var progressBarHeight: Float by OnLayoutProp(15F.pxValue())

    private var progressBarWidth:Float by OnLayoutProp(300F.pxValue())


    private var textSizeMarkers: Float by OnLayoutProp(12F.pxValue(TypedValue.COMPLEX_UNIT_SP)) {
        paintText.textSize = textSizeMarkers

    }

    private var markerColor: Int by OnValidateProp(Color.WHITE) {
        paintMarkers.color = markerColor
    }

    private var progressColor: Int by OnValidateProp(Color.GREEN) {
        paintProgress.color = progressColor
    }

    private var progressBackgroundColor: Int by OnValidateProp(Color.GRAY) {
    ...
    init {
        ...
        try {
            ...
            if (!markerString.isNullOrBlank()) {
                ...
                try {
                    input.map { if(it.toInt() in 1 .. totalProgress) this.markers.add(it.toInt()) }
                } catch (e: Exception) {
